 ...
 * uter
 *
 * When called with an input that should return no substrings at all, the
 function
 * should return an empty string, "".
 *
 * Note that the return type of the function is std::string, a C++ style string,
 * while the input is an
 * array of characters (equivalent to char*), which is a C-style string. We will
 do
 * a lot more with
 ...
 * cStyleString;
 * 3) For above to work as expected, the cStyleString should have a null
 character,
 * '\0', as its last character. So, when building the C-style string, allocate
 one
 * extra space
